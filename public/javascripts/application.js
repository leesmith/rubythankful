  ...
  $('dom.ready', function() {
    return $.getJSON("http://search.twitter.com/search.json?q=%23rubythankful&rpp=100&callback=?", function(search) {
      return $.each(search.results, function() {
        var linked_text;
        linked_text = this.text.replace(/(https?:\/\/[^ )]+)/i, "<a href='$1'>$1</a>").replace(/@([0-9a-z_]+)/gi, "<a href='http://twitter.com/$1'>@$1</a>").replace(/#([0-9a-z_-]+)/gi, "<a href='http://twitter.com/search?q=%23$1'>#$1</a>");
